...
namespace Booking3.UserControls
{
    public partial class HotelUC : UserControl
    {
        ...
        {
            //List<string> rating = SQLClass.Select(
            // "SELECT User, Hotel_ID, Rate, Comments FROM" + SQLClass.HOTELS +
            //" Comments = '" + textBox1.Text + "'");
            List<string> rating = SQLClass.Select(
                "SELECT User, Hotel_ID, Rate, Comments FROM 'rating'");

            SQLClass.Update(
                "UPDATE  " + SQLClass.HOTELS +
                " SET Comments = '" + textBox1.Text + "', " +
